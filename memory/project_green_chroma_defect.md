...
## 探针的可裁剪性预检（2026-09-22 追加）

`probe/probe_green_chroma.sh` 的目标尺寸改为参数 `OUT_W`/`OUT_H`（默认 768x432，即原始
缺陷口径），并在第 0 节加了**可裁剪性预检**：源两轴只要有一轴不大于目标、或源 == 目标
（crop 恒等）就**直接 exit 2**，不再静默跑出一轮"无产物/空"的结果。

起因：有人拿**已裁剪成品**（`*_Croped.mp4`，源本身就是 768x432）跑探针，目标也是
768x432，于是：

- 脚本的「同尺寸跳过」守卫（`vidcrop_hwaccel.py:4066`）先于 dry-run 打印（:4168）触发
  → 第 4 节 6 格全「无产物」、第 5 节一行命令都不打印、第 6 节抓不到命令串、连带 6b
  全组合穷举一并跳过；第 4 节的 `errline` 找不到真错误，退到关键字匹配后把能力探测行
  「CUDA 解码: 可用 ✓」当成错误行显示；
- 第 2/3 节的 `crop=768:432:0:0` 作用在 768x432 输入上是**恒等裁剪**，那一排「✓ 正常」
  对"crop 触发绿色"零判别力，尾判读的 ④「元凶在脚本多带的选项组」是假结论（该分支只
  校验手写命令，从未确认脚本复现了绿色）。

三处脚本调用（`scrun`、第 5 节、第 6 节抓 `CMD`）都补了 `--no-skip-same-size`（老
checkout 不支持时按 `--help` 探测后自动不传，不会把整节打成 unrecognized arguments）；
另修掉探针里裁剪 **x 偏移硬写 0** 的问题 —— 源比目标宽时它会与脚本
`_build_crop_filter_str` 的 `x=(SRC_W-OUT_W)/2` 不一致。

## 尾判读 ④ 分支按 V0 三分（2026-09-22 追加）

上一轮被误导的正是尾判读那句 ④：它的前提是「脚本原命令确实复现了绿色」，但 `AWK_VERDICT`
里根本没有这个输入 —— 只要手写命令（A0/B0/B1…）正常，它就在 `else` 里断言「元凶在脚本
多带的选项组」，**哪怕第 6 节的 V0 是正常的**（即本轮压根没复现故障）。

修法：第 6 节跑完 `runv V0_脚本原命令` 后立刻 `V0U=$LAST_U`，并作为 `-v v0=` 传进
`AWK_VERDICT`，④ 分支改成三分：

- `v0` 绿（<16）→ ④ 元凶在脚本那条命令多带的选项组（原意，现在有了确切前提）；
- `v0` 正常（>=16）→ ⚠ 手写与脚本原命令都没复现 → **本轮没有复现故障，别下结论**；
- `v0 < 0`（第 6 节没抓到命令、V0 未跑）→ ⚠ 无法定位，先查第 4 节 default 与第 5 节。

SELFTEST 增加了这三个子分支的断言（用 `a0=-1`/`b0=-1` 跳过 ⓪/⓪** 分支来精确命中 ④），
避免以后再退回"恒判 ④"。
