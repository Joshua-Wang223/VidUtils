...
A_CASES = [
    ...
    ("A5", "constqp --qp 0（0 档）", ["--codec", "hevc_nvenc", "--rc-mode", "constqp", "--qp", "0"],
     ["-rc constqp -qp 0", "-b:v 0"], ["-cq"], False,
     "只能单元级", "0 档形状不对（`-b:v 0` 是本轮补上的：显式 --qp 0 以前缺它）"),
    ("A6", "--cq 0 → 最高质量档改写", ["--codec", "hevc_nvenc", "--cq", "0"],
...
B_CASES = [
    ...
    ("B2", "constqp --qp 0 真跑通",
     ...
     "需上机", "0 档形状（-rc constqp -qp 0 -b:v 0）被 NVENC 拒绝"),
    ("B3", "默认路径真跑通（回归）",
     ["--codec", "hevc_nvenc"],
