#!/usr/bin/env bash
# =============================================================================
# nvinterpolate / minterpolate 2x 帧率 —— 崩溃安全版
#
# 做什么:
#   用光流插帧把输入帧率翻倍（目标帧率 = 源帧率 x2），音频原样 copy。
...
#
# 两条后端（--backend auto|gpu|cpu，默认 auto）:
#   gpu  nvinterpolate(NVOFA 光流) + hevc_nvenc —— 需要 NVIDIA GPU（Turing CC7.5+，驱动 >= 525）
#   cpu  minterpolate(运动补偿插帧) + libx265  —— 纯 CPU fallback
#   auto 先按 gpu 装配并【真跑一次试编码】；任一环节不可用就自动切 cpu 并打印原因。
#        试编码只读 4 帧，成本可忽略，但能把"驱动坏了 / CUDA 解码不了这个源 /
#        显存不足"拦在开始之前，而不是跑到第 30 片才炸。
#   ⚠ CPU 比 GPU 慢一到两个数量级（minterpolate 在 1080p 上大约是个位数 fps），
#     4K 素材基本不可行。长片请优先修好 GPU 环境。
#
...
#   · 负载：采样本 cgroup 的 cpu.stat（0.4s）算出"当前已被别的进程占用的核数"
#     —— 不能用 /proc/loadavg（容器里那是宿主的负载），并从可用核里扣掉。
#   · 其它实例：pgrep 找并发的 interp_2x_safe.sh，共用同一块 GPU 只会互相拖慢。
#
# 分片级并行（-j N）:
#   分片之间本来就互不依赖（各自 -ss + 独立滤镜实例 + 独立 TS），所以最多 N 片可以同时跑。
#   auto（默认）的取值：
#     · CPU 后端：CPU 槽位 = **可用核数 ÷ 每路 1 核**（= 可用核数本身；可用核数 = 本 cgroup
#       核数 - 正在被别的进程占用的核数），再与可用内存（按分辨率画像）取小者。
#       （CPU 后端**单片只吃 ~1 核** —— minterpolate 串行、libx265 的线程池也不受 -threads 限制，
#         所以"多开几路"是唯一有效的提速手段；每个 worker 自己的线程预算并不重要。）
#     · GPU 后端：**1**。依据 memory/project_t4_gpu_capabilities.md 的实测 —— T4 的 NVENC
...
#
# 前置条件:
#   · ffmpeg 带 nvinterpolate（GPU）或 minterpolate + libx265（CPU）
#     本机 = /usr/local/bin/ffmpeg 7.1，两套都有
...
#                        GPU 时是 nvenc 的 -cq，CPU 时作为 x265 的 -crf（不严格等价）
#       --backend NAME   auto(默认) / gpu / cpu。见开头"两条后端"
#       --cpu-preset N   CPU 后端时 libx265 的预设，默认 medium
#                        （ultrafast…veryslow；GPU 的 p1..p7 对 x265 无效）
#   -j, --jobs N         并行片数（别名 --workers）
#                          0 = 自动（默认）：CPU 后端按"可用核数（扣掉当前已被别人占用的）/ 2"
#                          与内存取小者；GPU 后端取 1
#                          1 = 顺序执行（与旧版本行为一致）
#                        auto 只影响速度，不影响分片内容，所以 **不写进 recipe**：
#                        改 -j 复用同一个 -w 目录时会照旧 skip，不会整体重编。
#       --threads N      每片 ffmpeg 线程数。0 = 自动（不显式限制），>0 才下发。
#                          ⚠ **CPU 后端真正有效的并行杠杆是 -j（多片并发），不是这个**：
#                            实测 minterpolate 是**串行**的（-filter_complex_threads 给 1/2/8 的
#                            wall 时间一样、都只吃 ~1.1 核）；而 -threads 对 libx265 **只改
#                            frame threads、不改线程池**（-threads 1 时 x265 仍打印
#                            "Thread pool created using 8 threads"，CPU 占用不降）。
#                          >0 现在会被翻译成 `-x265-params pools=N` —— 那才是真能限制
#                          x265 线程池的旋钮（实测 pools=2 → 2.28 核）。
#                          GPU 后端不下发（瓶颈在 GPU 引擎，不是 CPU）。
#       --mem-per-job GB 单任务内存估计，0 = 按分辨率画像（见 detect 段常量），
#                        仅用于算自动并行度
...
#   L  TOTAL_CAP  WORKDIR  PRESET  CQ  BACKEND  CPU_PRESET  JOBS  THREADS  MEM_PER_JOB
...
#   第一层 recipe.txt：记 backend|enc|in|L|trim。这些参数一变，目录里
#     **每一片**的内容都会不同（帧边界变了/画质档变了/换了视频/换了后端）→ 整体拒绝，
...
#
#   # 9) 强制走 CPU（想复现/对比，或 GPU 正被别人占着）
#   bash /workspace/VidUtils/interp_2x_safe.sh /path/in.mp4 --backend cpu --cpu-preset ultrafast
#
...
用法: bash interp_2x_safe.sh <输入视频> [输出路径] [选项]
  ...
  默认先试 GPU（nvinterpolate + hevc_nvenc），不可用则自动回退 CPU
  （minterpolate + libx265，慢很多）。
...
选项:
  ...
  -p, --preset NAME   hevc_nvenc 预设，默认 p5（越大越慢、同码率画质越好）
  -c, --cq N          恒定质量，默认 25（越大越省码率、画质越低）
                      GPU → nvenc -cq；CPU → x265 -crf
      --backend NAME  auto(默认) / gpu / cpu；auto 会在开工前试编码，
...
环境自动探测（开工前打印）:
  ...
  其它 interp_2x_safe.sh 实例。
  还会采样本 cgroup 的 cpu.stat（0.4s）算出"当前已被别的进程占用的核数"，并把它
  从可用核里扣掉再算 -j auto（避免和正在跑的任务互相抢）。
...
BACKEND_OPT=""; CPU_PRESET_OPT=""
JOBS_OPT=""; THREADS_OPT=""; MEM_PER_JOB_OPT=""
SS_OPT=""; TO_OPT=""; T_OPT=""
SEQUENTIAL=false
OVERWRITE=false
...
while (( $# > 0 )); do
    case "$1" in
        ...
        --backend)      need_arg "$@"; BACKEND_OPT=$2; shift 2 ;;
        --cpu-preset)   need_arg "$@"; CPU_PRESET_OPT=$2; shift 2 ;;
        -j|--jobs|--workers) need_arg "$@"; JOBS_OPT=$2; shift 2 ;;
        --threads)      need_arg "$@"; THREADS_OPT=$2; shift 2 ;;
        --mem-per-job)  need_arg "$@"; MEM_PER_JOB_OPT=$2; shift 2 ;;
...
CQ=${CQ_OPT:-${CQ:-25}}                  # 恒定质量（GPU: -cq / CPU: -crf）
BACKEND=${BACKEND_OPT:-${BACKEND:-auto}} # auto | gpu | cpu
CPU_PRESET=${CPU_PRESET_OPT:-${CPU_PRESET:-medium}}   # libx265 预设（仅 CPU 后端）
...
MEM_PER_JOB_RAW=${MEM_PER_JOB_OPT:-${MEM_PER_JOB:-0}}
...
#   · GPU: nvinterpolate 2x 的**前 3 个输出帧是同一张画面重复** —— 光流要"前一帧"才能算，
#     实例刚起来时吐不出有效插值。实测：这 3 帧对源第 1 帧 ≈44.6dB、对源第 0 帧只 20dB，
#     第 4 个输出帧才是真正插出来的 → 必须剪掉
#   · CPU: minterpolate 没有这种预热帧，第 0 帧就是源第 0 帧本身（实测 ≈44.3dB），不需要剪
# 这个值会写进 recipe.txt —— 两种后端的分片画面不同，不允许混在一个目录里复用。
GPU_HEAD_TRIM=3
# 注意（GPU）：-t 是【输出】选项，剪掉 3 帧后 ffmpeg 会自己多读 3 个源帧把这片补满 L 秒，
...
case "$BACKEND" in
    auto|gpu|cpu) ;;
    *) die "BACKEND 只能是 auto / gpu / cpu，当前为 '$BACKEND'" ;;
esac
...
case "$MEM_PER_JOB_RAW" in
    ''|*[!0-9.]*|*.*.*) die "MEM_PER_JOB 必须是非负数字（GB），当前为 '$MEM_PER_JOB_RAW'" ;;
esac
JOBS_OPT=$(( 10#$JOBS_RAW ))            # 10# 前缀：让 -j 08 也能当十进制解析
THREADS_OPT=$(( 10#$THREADS_RAW ))
MEM_PER_JOB_OPT=$(awk -v v="$MEM_PER_JOB_RAW" 'BEGIN{printf "%.2f", v+0}')
...
# --- 源帧率与尺寸（一次 ffprobe 拿全；高度用于并行度的内存/显存画像） ----------
# 源帧率 → 目标帧率（x2）。GPU 那边 nvinterpolate 支持直接写 fps=source_fps*2，
# 但 minterpolate 的 fps 只吃具体数值，写 source_fps*2 会报
#   "Unable to parse option value ... as video rate"
# 所以这里统一算出来，CPU 后端直接用。
...
# CPU 后端在片尾补的克隆帧时长 = 2 个源帧（见 setup_cpu 的 tpad 说明）
TAIL_PAD=$(awk -v r="$SRC_RATE" 'BEGIN{
        split(r, a, "/"); f = a[1] / (a[2] == "" ? 1 : a[2]); printf "%.4f", 2/f }')

# --- 时长 / 分片数 -----------------------------------------------------------
# 放在后端选择【之前】：并行度要先夹到 NPARTS（片数比核数还少时开那么多路没意义）。
...
# 给 ffmpeg 用的输入定位参数。
#   SS_OPTS     —— 视频分片 / GPU 试编码的起点（= --SS，SEG_START）
...
# 当前【已经被别的进程用掉】的核数：采样本 cgroup 的 cpu.stat，**不要**用 /proc/loadavg
# —— 容器里 loadavg 报的是宿主的负载，拿来判并行度会完全跑偏（和 nproc 是同一个坑）。
# delta(usage_usec) / delta(微秒) 正好等于这个 cgroup 内所有进程实际消耗的核数；
# 启动阶段我们自己还没派发编码任务，所以量到的基本就是"别人"占的。
# 拿不到 cpu.stat（老 cgroup / 非容器）返回非 0，调用方按"不看负载"处理（= 旧行为）。
CPU_USED_CORES=""
detect_cpu_load() {
    local f=/sys/fs/cgroup/cpu.stat u0 u1 t0 t1
    [[ -r "$f" ]] || return 1
    u0=$(awk '/^usage_usec/{print $2; exit}' "$f" 2>/dev/null || true)
    case "$u0" in ''|*[!0-9]*) return 1 ;; esac
    t0=$(date +%s%N)
    sleep 0.4
    u1=$(awk '/^usage_usec/{print $2; exit}' "$f" 2>/dev/null || true)
    case "$u1" in ''|*[!0-9]*) return 1 ;; esac
    t1=$(date +%s%N)
    CPU_USED_CORES=$(awk -v a="$u0" -v b="$u1" -v ta="$t0" -v tb="$t1" 'BEGIN{
        dt = (tb - ta) / 1000;                 # 纳秒 → 微秒
        if (dt <= 0 || b < a) exit 1
        printf "%.2f", (b - a) / dt }') || { CPU_USED_CORES=""; return 1; }
    [[ -n "$CPU_USED_CORES" ]] || return 1
    return 0
}
...
detect_mem
detect_disk
detect_cpu_load || CPU_USED_CORES=""
log "环境  : CPU 核 ${CPU_CORES} (${CPU_SRC})  MEM 总 ${MEM_TOTAL_GB}GB 可用 ${MEM_AVAIL_GB}GB (${MEM_SRC})"
if [[ -n "$CPU_USED_CORES" ]]; then
    log "负载  : cgroup 内其它进程已用 ≈${CPU_USED_CORES} 核（cpu.stat 采样 0.4s，本 cgroup 共 ${CPU_CORES} 核）"
fi
...
# --backend cpu 时不查 GPU：省一次 nvidia-smi，也不打会让人误解的 GPU 行
if [[ "$BACKEND" != cpu ]]; then
    if detect_gpu_info; then
        ...
        case "$GPU_VRAM_FREE_MB" in
            ...
            *) if (( GPU_VRAM_FREE_MB < 1536 )); then
                   log "提示  : 显存只剩 ${GPU_VRAM_FREE_MB}MB，试编码很可能失败并回退 CPU"
               fi ;;
        esac
    fi
fi

# --- 后端装配：GPU / CPU 两套参数都在这里定好，下面只按 $BACKEND 取用 -----------
# $HEAD_TRIM / $FC / $DEC_OPTS / $ENC_OPTS 必须成套切换：混用会让同一个目录里
# 出现内容不一致的分片（recipe.txt 里记了 backend 与 trim，会拒绝混用）。
setup_gpu() {
    HEAD_TRIM=$GPU_HEAD_TRIM
...
# CPU 后端的视频编码参数。**要在 THREADS 定下来之后拼** —— 因为它把 THREADS 翻成
# `-x265-params pools=N`：这才是唯一真能限住 x265 线程池的旋钮（实测 pools=2 → 2.28 核）；
# 而 `-threads` 只设 frame threads、线程池仍是满核（-threads 1 时 x265 照样打印
# "Thread pool created using 8 threads"）。THREADS=0（未定/GPU）时就不加 pools。
build_cpu_enc_opts() {
    local x265p="log-level=error"
    if (( ${THREADS:-0} > 0 )); then
        x265p="pools=${THREADS}:${x265p}"
    fi
    # -cq 是 nvenc 的叫法，x265 对应 -crf；数值不严格等价，只是同一个"越大越省"旋钮。
    # log-level=error：x265 默认往 stderr 打一堆 [info]，会把进度日志淹掉。
    ENC_OPTS=(-c:v libx265 -preset "$CPU_PRESET" -crf "$CQ" -x265-params "$x265p")
}

setup_cpu() {
    HEAD_TRIM=0
    DEC_OPTS=()
    # tpad 是必需的，不是保险: minterpolate 吐不出"最后一帧"（没有下一帧可插），
    # 撞到 EOF 时输出会凭空少 3 帧 —— 整片单跑也一样（实测 8s/24fps 源: 应 384 帧
    # 实得 381）。末尾补 2 个源帧的克隆帧给它当"下一帧"，多出来的再由 -t 裁掉。
    # 开销 = 每片末尾多插值 2 帧，可忽略。
    FC="tpad=stop_mode=clone:stop_duration=${TAIL_PAD},minterpolate=fps=${TARGET_RATE},setpts=PTS-STARTPTS"
    # 编码参数交给 build_cpu_enc_opts 拼（它还要看 THREADS → pools=N）。
    # 这里先拼一次"没有 THREADS"的版本；JOBS/THREADS 定下之后 set_threads_from_jobs 会重拼。
    build_cpu_enc_opts
    have_filter minterpolate \
        || die "CPU 回退需要 minterpolate 滤镜，$FFMPEG 里没有"
    have_encoder libx265 \
        || die "CPU 回退需要 libx265 编码器，$FFMPEG 里没有"
}
...
case "$BACKEND" in
    gpu)
        setup_gpu
        gpu_probe_adjust || die "指定了 --backend gpu，但 GPU 不可用: $GPU_WHY
  去掉该选项即可自动回退 CPU（慢很多）；或先修好驱动 / ffmpeg / 显存占用。" ;;
    cpu)
        setup_cpu ;;
    auto)
        setup_gpu
        if gpu_probe_adjust; then
            BACKEND=gpu
        else
            BACKEND=cpu
            setup_cpu
            log "GPU 不可用（$GPU_WHY）→ 自动回退 CPU（minterpolate + libx265，比 GPU 慢很多）"
        fi ;;
esac

# --- 音轨起点偏移（收尾 copy 用）----------------------------------------------
# 放在后端装配【之后】：HEAD_TRIM 由 setup_gpu/setup_cpu 设定（GPU=3、CPU=0）。
...
# CPU 后端 HEAD_TRIM=0 → AUD_SS_OPTS 与 SS_OPTS 等价，命令与旧版逐字节一致。
...
# ------------------------------- 并行度 --------------------------------------
# 画像与常量（可被 -j / --threads / --mem-per-job 覆盖）：
#   CPU_PREF_THREADS=1   每路按 **1 核**估（收敛过程 4 → 2 → 1）。依据（同机实测）：
#                        · **按线程采样**一个在跑的 4K minterpolate 片：25 个线程里**只有 1 个在
#                          满转（1.01 核），其余 24 个 ≈0**，全进程合计 1.01 / 8 核 ——
#                          minterpolate 是**串行**的，所以单片就是 ~1 核；
#                        · lab（1080p 单分片，指标 (user+sys)/wall）：-filter_complex_threads
#                          1/2/8 → wall 35.9/36.8/36.1s、并行度 1.09~1.12 核（对滤镜无效）；
#                          -threads 1/2/8（只有 x265）→ 并行度 2.92/2.93/3.09 核（只改
#                          frame threads，线程池仍是 8，只有 -x265-params pools=2 真降到 2.28）。
#                        · 1080p 基准 -j 1/2/4/8 = 85.7/49.1/27.2/27.6s（那轮有背景负载，
#                          平台偏低；单片只吃 1 核，理论上还能更高）。
#                        → 按 1 核/路把可用核铺满，通常由**内存画像**封顶（4K 4GB/片 ≈ 5 路）。
#   MEM_RESERVE_GB=0.2 / MEM_USABLE_RATIO=0.80  同 v2 的 _MIN_SYSTEM_RESERVE_GB / _MAX_USABLE_RATIO
#   GPU_JOBS_AUTO=1      见文件头：T4 实测 NVENC 单引擎，多路并发总吞吐基本不变
CPU_PREF_THREADS=1
MEM_RESERVE_GB=0.2
MEM_USABLE_RATIO=0.80
GPU_JOBS_AUTO=1

# 单任务内存画像（GB）：minterpolate 的帧/光流缓冲 + x265 的线程池。
# 实测（`/usr/bin/time -v` 的 Maximum resident set size，单分片、-threads 4、libx265 medium）：
#   · 1280x720（new5_10s.mp4）        → 0.61 GB
#   · 1920x1080（合成 + new5_raw.mp4）→ 1.16 / 1.17 GB
#   · 3840x2160（Kangaroo 4K 源）     → **4.59 GB**（把 x265 限到 pools=1 时 4.25 GB）
# 拟合：m ≈ 0.11 + 0.54 × 百万像素（1080p→1.23、1440p→2.10，与旧值 1.2/2.0 吻合）。
# 下面取「实测 × ~1.1」留余量；再高的分辨率按像素外推（8K ≈ 18GB）。
# ⚠ 4K 这格原来是 4.0，比实测低 ~15% —— 大内存机器上会把 -j 开到略超（OOM 风险），故改为 5.0。
mem_per_job_profile() {
    local h=${1:-0}
    if   (( h <= 720 ));  then printf '0.7'
    elif (( h <= 1080 )); then printf '1.3'
    elif (( h <= 1440 )); then printf '2.3'
    elif (( h <= 2160 )); then printf '5.0'
    else printf '18.0'; fi
}
...
MEM_PER_JOB_GB=$(mem_per_job_profile "$SRC_H")
if awk -v v="$MEM_PER_JOB_OPT" 'BEGIN{exit !(v > 0)}'; then
    MEM_PER_JOB_GB=$MEM_PER_JOB_OPT
fi
...
if (( JOBS_OPT > 0 )); then
    ...
    # CPU 槽位：把"别人已经占用的核数"从可用核里扣掉 —— 否则会和正在跑的任务互相踩
    # （8 核配额的机器上已经跑着一个 4 路任务时，我们再按 8 核铺 4 路 = 8 路抢 8 核，
    # 结果一起变慢）。至少留一路的量，别把自己算成 0 路。
    CPU_SLOTS=$(awk -v c="$CPU_CORES" -v u="${CPU_USED_CORES:-0}" -v p="$CPU_PREF_THREADS" 'BEGIN{
        free = c - u; if (free < p) free = p; n = int(free / p); if (n < 1) n = 1; print n }')
    USABLE_MEM_GB=$(awk -v t="$MEM_TOTAL_GB" -v a="$MEM_AVAIL_GB" \
        -v r="$MEM_RESERVE_GB" -v u="$MEM_USABLE_RATIO" \
        'BEGIN{ x = t*u; y = a-r; if (y < x) x = y; if (x < 0.1) x = 0.1; printf "%.2f", x }')
    MEM_SLOTS=$(awk -v u="$USABLE_MEM_GB" -v m="$MEM_PER_JOB_GB" \
        'BEGIN{ n = int(u/m); if (n < 1) n = 1; print n }')
    JOBS=$(( CPU_SLOTS < MEM_SLOTS ? CPU_SLOTS : MEM_SLOTS ))
    if [[ -n "$CPU_USED_CORES" ]]; then
        FREE_CORES=$(awk -v c="$CPU_CORES" -v u="$CPU_USED_CORES" 'BEGIN{ f = c-u; if (f<0) f=0; printf "%.1f", f }')
        JOBS_SRC="CPU 槽位 ${CPU_SLOTS}（${CPU_CORES} 核 - 已用 ${CPU_USED_CORES} 核 = 剩 ${FREE_CORES} 核，每路 ${CPU_PREF_THREADS} 核）× 内存 ${MEM_SLOTS}，每任务 ${MEM_PER_JOB_GB}GB / 可用 ${USABLE_MEM_GB}GB"
    else
        JOBS_SRC="CPU 槽位 ${CPU_SLOTS}（${CPU_CORES} 核 / 每路 ${CPU_PREF_THREADS} 核，未取到负载）× 内存 ${MEM_SLOTS}，每任务 ${MEM_PER_JOB_GB}GB / 可用 ${USABLE_MEM_GB}GB"
    fi
fi
if (( JOBS > NPARTS )); then JOBS=$NPARTS; fi
if (( JOBS < 1 )); then JOBS=1; fi

# 每片线程数（THREADS）。⚠ 别指望它"把核分给各片" —— 实测：
#   · minterpolate 是**串行**的：-filter_complex_threads 给 1/2/8，wall 时间一样（~36s）、
#     都只吃 ~1.1 核 → 这个选项对插帧滤镜无效（照发只是为了把预算写清楚，别无他用）；
#   · -threads 对 libx265 只设 frame threads，**不改 x265 的线程池**（-threads 1 时
#     x265 仍打印 "Thread pool created using 8 threads"，CPU 占用不降）；
#     真要限 x265 得用 `-x265-params pools=N`（pools=2 → 实测 2.28 核）。
#   · 所以 CPU 后端**真正的并行杠杆是 -j**（单片 ≈1 核，靠多片并发吃满核）。
# THREADS 的用途：把核数在 N 路之间摊开 —— 下发 -threads/-filter_complex_threads，
# 并在算完之后把 `pools=$THREADS` 拼进 -x265-params（见 build_cpu_enc_opts）。
# GPU 后端不下发（瓶颈在 GPU 引擎，锁 CPU 线程数只会拖慢喂数据）。
# 抽成函数：JOBS 在 TODO 扫描之后还会因为"实际要编几片"再收敛一次，那时 THREADS 要跟着回算。
set_threads_from_jobs() {
    if (( THREADS_OPT > 0 )); then
        THREADS=$THREADS_OPT
    elif [[ "$BACKEND" == cpu ]]; then
        THREADS=$(( CPU_CORES / JOBS )); if (( THREADS < 1 )); then THREADS=1; fi
    else
        THREADS=0
    fi
    ...
    # THREADS 现在定下来了 → CPU 后端把它翻进 -x265-params pools=N（GPU 后端不碰，ENC_OPTS 是 nvenc 的）
    if [[ "$BACKEND" == cpu ]]; then build_cpu_enc_opts; fi
    return 0
}
set_threads_from_jobs

# 用户显式开多路时，对照画像给个"建议值"，只告警不拦 —— 他要压满机器是他的选择。
if [[ "$BACKEND" == gpu ]] && (( JOBS > 1 )); then
...
if [[ "$BACKEND" == gpu ]]; then
    ...
    ENC_TAG="libx265|$CPU_PRESET|crf=$CQ"
fi
RECIPE="slice=3|backend=$BACKEND|enc=$ENC_TAG|in=$IN|L=$L|trim=$HEAD_TRIM"
...
# 加 backend 字段之前的格式：当时只可能是 GPU 跑出来的，遇到它就按 gpu 就地升级，
# 免得已经在跑的长任务因为脚本升级被迫从头重编。
LEGACY_RECIPE="slice=3|in=$IN|L=$L|preset=$PRESET|cq=$CQ|trim=$GPU_HEAD_TRIM"
STAMP="$WORKDIR/recipe.txt"
if [[ -f "$STAMP" ]]; then
    OLD_RECIPE=$(cat "$STAMP")
    if [[ "$OLD_RECIPE" == "$RECIPE" ]]; then
        :
    elif [[ "$OLD_RECIPE" == "$LEGACY_RECIPE" && "$BACKEND" == gpu ]] && ! is_pos "$SEG_START"; then
        printf '%s\n' "$RECIPE" > "$STAMP"
  ...
  已有 : $OLD_RECIPE
  本次 : $RECIPE
换输入/换后端/换 --SS 起点请另给 WORKDIR，或删掉该目录后重跑。"
    fi
else
    printf '%s\n' "$RECIPE" > "$STAMP"
fi
...
# CPU 后端 HEAD_TRIM=0 时它与 SS_OPTS 等价，命令与旧版逐字节一致。
