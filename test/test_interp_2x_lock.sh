...
# =============================================================================
echo
echo "=== 用例 6：实例内并行（-j 2）—— 结果要与顺序模式等价，且不影响断点复用 ==="
# 与用例 1~5 守的"实例之间"互斥不同，这条守的是"实例内部多路并行"：
# 分片之间互不依赖，可以同时跑，但产出必须与顺序模式完全一致，
# 而且并行度不参与 recipe 指纹（改 -j 不能触发整体重编）。
W=$TMPROOT/t6; OUT="$W/out.mp4"
bash "$SUT" "$IN" "$OUT" -w "$W" -L "$SEG" -j 2 >"$W.p.log" 2>&1; rcP=$?
assert_eq "并行 -j 2 exit code" "$rcP" "0"
assert_re "日志里确实用了 2 路并行" '并发 +: +2 路' "$W.p.log"
assert_file "并行产出成片" "$OUT"
assert_near "并行成片帧数" "$(frames "$OUT")" "$EXPECT"
assert_eq "并行分片数" "$(count_ts "$W")" "$NPARTS"
assert_eq "并行 meta 数" "$(count_meta "$W")" "$NPARTS"
# 每片的切法必须仍是"该有的那个"（与顺序模式同一套断言，等价于逐片比对）
assert_metas_consistent "6: 并行 -j 2" "$W"
assert_eq "并行没有残留临时文件" "$(count_temp "$W")" "0"
assert_eq "并行确实把每一片都编了（run 数）" "$(n_run "$W.p.log")" "$NPARTS"

# 并行度不进 recipe：换回 -j 1 必须全部走 skip、零编码
FP6=$(parts_fp "$W")
bash "$SUT" "$IN" "$OUT" -w "$W" -L "$SEG" -j 1 --overwrite >"$W.re.log" 2>&1; rcR=$?
assert_eq "换 -j 1 复用 exit code" "$rcR" "0"
assert_ngrep "换 -j 没有被 recipe 拒绝" "拒绝复用" "$W.re.log"
assert_eq "换 -j 后全部走 skip" "$(n_skip "$W.re.log")" "$NPARTS"
assert_eq "换 -j 后零编码（run 数）" "$(n_run "$W.re.log")" "0"
assert_eq "换 -j 后分片指纹未变" "$(parts_fp "$W")" "$FP6"

# 并行模式下删掉一片 → 只重做那一片（其余照旧复用，成片补齐）
rm -f "$W/parts/p00001.ts" "$W/parts/p00001.meta"
bash "$SUT" "$IN" "$OUT" -w "$W" -L "$SEG" -j 2 --overwrite >"$W.re2.log" 2>&1; rcR2=$?
assert_eq "补片 exit code" "$rcR2" "0"
assert_eq "只重做缺的那一片（run 数）" "$(n_run "$W.re2.log")" "1"
assert_re "重做的正是 p00001" 'run +p00001' "$W.re2.log"
assert_eq "其余片全部复用（skip 数）" "$(n_skip "$W.re2.log")" "$(( NPARTS - 1 ))"
assert_eq "补片后分片数" "$(count_ts "$W")" "$NPARTS"
assert_near "补片后成片帧数" "$(frames "$OUT")" "$EXPECT"
